...
public class NonFoodProduct extends Product{
    ...
    public double getPrice(int year, int month) {
        if(year < 2010) throw new IndexOutOfBoundsException();
        else if(year > 2021) {
            if(year==2022) {
                if (month > 3) throw new IndexOutOfBoundsException();
            }else throw new IndexOutOfBoundsException();
        }
        if(month<1) throw new IndexOutOfBoundsException();
        else if(month>12) throw new IndexOutOfBoundsException();
        else {
            return prices[((year-2010)*12)+month];
        }
    }
}
